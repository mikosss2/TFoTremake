var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">Time Speed: {{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}} Time / sec</span>
    ...
    'info-tab': {
        ...
        Prestige Time: {{ formatTime(player["f"].realtime) }}<br><br>
        <h3>Hotkeys</h3><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
				<td><button class="opt" onclick="toggleOpt('nodeStyle')">Node Style: {{ options.nodeStyle?"SQUARES":"CIRCLES" }}</button></td>
			</tr> 
        </table>`
    },
